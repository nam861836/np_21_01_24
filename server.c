...
char *add_character_every_four(const char *input_string, char separator);
...
char *generate_random_string(char *random_string, size_t length);
...
int book_ticket(sqlite3 *db, const char *ticket_code, int user_id, const char *flight_num, const char *seat_class, int num_seat, float ticket_price);
        ...
        if (sscanf(buffer, "%s %s %s", command, username, password) == 3)
        {
            ...
            else if (strncmp(buffer, "delay", 5) == 0)
            {
                if (user_id != -1 && user_type == 1)
                {
                    ...
                    if (execute_query(db, update_query) == SQLITE_OK)
                    {
                        ...
                        send_message_to_all_clients("Flight has been delayed\n", client_socket);
                ...
                if (user_id != -1 && user_type == 2)
                {
                    if (sscanf(buffer, "book %s %s %d", flight_num, seat_class, &num_seat) == 3)
                    {
                        if (strcmp(seat_class, "A") == 0 || strcmp(seat_class, "B") == 0)
                        {
                            ...
                            {
                                char ticket_code[50], str[50];
                                ticket_price = num_seat * get_ticket_price(db, flight_num, seat_class);

                                strcat(ticket_code, add_character_every_four(generate_random_string(str, 4 * num_seat), '-'));
            ...
            else if (strncmp(buffer, "see", 3) == 0)
            {
                if (user_id != -1 && user_type == 2)
                {
                    ...
                    {
                        char result[10000] = "";
                        strcpy(result, "Notifications found:\n");
                        ...
                        if (strcmp(result, "Notifications found:\n") == 0)
                        {
...
char* add_character_every_four(const char *input_string, char separator)
{
    ...
    size_t new_length = input_length + input_length / 4; // Calculate the new length with separators
    char *result_string = (char *)malloc(new_length + 1); // +1 for the null terminator
    ...
    if (result_string != NULL)
    {
        ...
        {
            result_string[result_index++] = input_string[i];

            // Add separator after every four characters (except for the last one)
            if ((i + 1) % 4 == 0 && i + 1 != input_length)
            {
                result_string[result_index++] = separator;
            }
        }

        result_string[new_length] = '\0'; // Null-terminate the result string
    }
...
int get_noti_from_user(sqlite3 *db, const char *query, int *user_id)
{
    ...
    if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) == SQLITE_OK)
    {
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            if (num_noti < 100)
            {
                user_id[num_noti++] = sqlite3_column_int(stmt, 0);
            }
            else
                break;
        }
...
char *generate_random_string(char *random_string, size_t length)
{
    ...
    static unsigned int seed = 0; // Static variable to store the seed
    // Update the seed based on current time and seat number
